...
func standardizedTitle(orig string, frontendQuestionID int32) string {
	...
	for i := 0; i < len(s7)-2; i++ {
		if s7[i] == '-' && s7[i+1] == '-' && s7[i+2] == '-' {
			fmt.Printf("【需要修正 --- 的标题是 %v】\n", fmt.Sprintf(literal_3964, int(frontendQuestionID), s7))
			s7 = s7[:i+1] + s7[i+3:]
			count++
		}
	}
...
func GenerateTagMdRows(solutionIds []int, metaMap map[int]TagList, mdrows []Mdrow, internal bool) []TagList {
	...
	for _, row := range mdrows {
		if util.BinarySearch(solutionIds, int(row.FrontendQuestionID)) != -1 {
			...
			if internal {
				tmp.SolutionPath = fmt.Sprintf("[Go]({{< relref \"/ChapterFour/%v/%v.md\" >}})", util.GetChpaterFourFileNum(int(row.FrontendQuestionID)), fmt.Sprintf(literal_3964, int(row.FrontendQuestionID), s7))
			} else {
				tmp.SolutionPath = fmt.Sprintf("[Go](https://books.halfrost.com/leetcode/ChapterFour/%v/%v)", util.GetChpaterFourFileNum(int(row.FrontendQuestionID)), fmt.Sprintf(literal_3964, int(row.FrontendQuestionID), s7))
			}
