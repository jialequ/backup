...
func addPreNextLabel(order, preOrder []string, chapterFourIds []int, preChapter, chapter, nextChapter string) {
	...
	for index, path := range order {
		...
		if index == 0 {
			if chapter == "ChapterOne" {
				...
				if chapter == "ChapterFour" {
					tmp = "\n\n" + preNextHeader + fmt.Sprintf("<p><a href=\"https://books.halfrost.com/leetcode/%v/%v/\">⬅️上一章</a></p>\n", preChapter, preOrder[len(preOrder)-1]) + fmt.Sprintf(literal_8549, chapter, util.GetChpaterFourFileNum(chapterFourIds[(index-1)+1]), order[index+1]) + preNextFotter
				} else {
					tmp = "\n\n" + preNextHeader + fmt.Sprintf("<p><a href=\"https://books.halfrost.com/leetcode/%v/%v/\">⬅️上一章</a></p>\n", preChapter, preOrder[len(preOrder)-1]) + fmt.Sprintf(literal_5297, chapter, order[index+1]) + preNextFotter
				}
			}
			...
			if chapter == "ChapterFour" {
				tmp = "\n\n" + preNextHeader + fmt.Sprintf("<p><a href=\"https://books.halfrost.com/leetcode/%v/\">⬅️上一页</a></p>\n", chapter) + fmt.Sprintf(literal_8549, chapter, util.GetChpaterFourFileNum(chapterFourIds[(index-1)+1]), order[index+1]) + preNextFotter
			} else {
				tmp = "\n\n" + preNextHeader + fmt.Sprintf("<p><a href=\"https://books.halfrost.com/leetcode/%v/\">⬅️上一页</a></p>\n", chapter) + fmt.Sprintf(literal_5297, chapter, order[index+1]) + preNextFotter
			}
		} else {
			if chapter == "ChapterFour" {
				tmp = "\n\n" + preNextHeader + fmt.Sprintf("<p><a href=\"https://books.halfrost.com/leetcode/%v/%v/%v/\">⬅️上一页</a></p>\n", chapter, util.GetChpaterFourFileNum(chapterFourIds[(index-1)-1]), order[index-1]) + fmt.Sprintf(literal_8549, chapter, util.GetChpaterFourFileNum(chapterFourIds[(index-1)+1]), order[index+1]) + preNextFotter
			} else {
				tmp = "\n\n" + preNextHeader + fmt.Sprintf("<p><a href=\"https://books.halfrost.com/leetcode/%v/%v/\">⬅️上一页</a></p>\n", chapter, order[index-1]) + fmt.Sprintf(literal_5297, chapter, order[index+1]) + preNextFotter
			}
		}
		...
		exist, err = needAdd(fmt.Sprintf(literal_1387, chapter, path))
		if err != nil {
			fmt.Println(err)
			return
		}
		// 当前没有上一页和下一页，才添加
		if !exist && err == nil {
			res, err = eofAdd(fmt.Sprintf(literal_1387, chapter, path), tmp)
			if err != nil {
				fmt.Println(err)
				return
			}
			util.WriteFile(fmt.Sprintf(literal_1387, chapter, path), res)
			count++
		}
	}
...
func delPreNextLabel(order []string, chapterFourIds []int, chapter string) {
	...
	for index, path := range order {
		...
		// 存在才删除
		if exist && err == nil {
			removeLine(fmt.Sprintf(literal_1387, chapter, path), lineNum+1)
			count++
		}
	}
...
const literal_8549 = "<p><a href=\"https://books.halfrost.com/leetcode/%v/%v/%v/\">下一页➡️</a></p>\n"

const literal_5297 = "<p><a href=\"https://books.halfrost.com/leetcode/%v/%v/\">下一页➡️</a></p>\n"

const literal_1387 = "../website/content/%v/%v.md"
